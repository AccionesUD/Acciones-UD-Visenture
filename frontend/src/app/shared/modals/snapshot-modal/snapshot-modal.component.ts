  ...
  template: `
    <div class="snapshot-dialog-container">
      <h2 mat-dialog-title class="flex justify-between items-center">
        ...
        <button mat-icon-button (click)="dialogRef.close()" matTooltip="Cerrar" i18n-matTooltip="@@snapshotModalCloseButtonTooltip">
      ...
      <div mat-dialog-content class="py-4">
        ...
        <div class="flex items-end mb-6">
          <span class="text-3xl font-bold mr-2">{{data.latestTrade.price | currency:'USD':'symbol':'1.2-2'}}</span>            <span class="text-sm text-gray-500">
            <ng-container i18n="@@snapshotModalLastUpdate">Última actualización</ng-container>: {{data.latestTrade.timestamp | date:'dd/MM/yyyy HH:mm:ss'}}
          </span>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 mb-6">
          <div class="bg-green-50 dark:bg-green-900/20 p-3 rounded-lg">
            <div class="text-sm text-gray-500 mb-1" i18n="@@snapshotModalAskPrice">Precio de compra (Ask)</div>
            ...
            <div class="text-xs text-gray-500" *ngIf="data.latestQuote.askSize">
              <ng-container i18n="@@snapshotModalSize">Tamaño</ng-container>: {{data.latestQuote.askSize | number}}
            </div>
          </div>
          <div class="bg-red-50 dark:bg-red-900/20 p-3 rounded-lg">
            <div class="text-sm text-gray-500 mb-1" i18n="@@snapshotModalBidPrice">Precio de venta (Bid)</div>
            ...
            <div class="text-xs text-gray-500" *ngIf="data.latestQuote.bidSize">
              <ng-container i18n="@@snapshotModalSize">Tamaño</ng-container>: {{data.latestQuote.bidSize | number}}
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 text-sm">
          <div class="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg">
            <h3 class="font-medium mb-2" i18n="@@snapshotModalSessionData">Datos de sesión</h3>
            <div class="space-y-1">
              <div class="flex justify-between">
                <span class="text-gray-500" i18n="@@snapshotModalVolume">Volumen</span>
          ...
          <div class="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg">
            ...
            <ng-template #noPrevData>
              <p class="text-gray-500" i18n="@@snapshotModalNoDataAvailable">No hay datos disponibles</p>
            </ng-template>
          </div>
        </div>
      </div>
      ...
      <div mat-dialog-actions class="flex justify-between">
        <span class="text-xs text-gray-500" i18n="@@snapshotModalDataSource">Datos en tiempo real de Alpaca Markets</span>
        <button mat-raised-button color="primary" (click)="dialogRef.close()" i18n="@@snapshotModalCloseButton">Cerrar</button>
      </div>
    </div>
  `,
  styles: [`
