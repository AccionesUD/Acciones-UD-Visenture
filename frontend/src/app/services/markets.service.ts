...
export class MarketServiceService { // Manteniendo el nombre de clase original del archivo  private apiUrl = `${environment.apiUrl}/markets`; // URL para endpoints de mercados
  private marketsSubject = new BehaviorSubject<Market[] | null>(null);
  public markets$ = this.marketsSubject.asObservable();

  // Datos simulados para usar mientras no esté implementado el backend
  private simulatedMarkets: Market[] = [
    ...
    { id: 'BMV', name: 'Bolsa Mexicana de Valores', country: 'México', currency: 'MXN', status: 'open' }
  ];
