...
export class StopSell implements IOrderTypeStrategy {
    constructor(
    ...
    async valid(): Promise<void> {
        ...
        if (stop_price >= quoteAmmount.quotes[symbol].ap) {
            throw new BadRequestException('El valor stop_price no puede ser mayor al precio de mercado de la accion, o se ejecutara como market')
        }
